...
export function getBlocksCount(
  ...
  const key = (version - 1) * 4;
  if (errorCorrectionLevel === L) return EC_BLOCKS_TABLE[key + 0];
  if (errorCorrectionLevel === M) return EC_BLOCKS_TABLE[key + 1];
  if (errorCorrectionLevel === Q) return EC_BLOCKS_TABLE[key + 2];
  if (errorCorrectionLevel === H) return EC_BLOCKS_TABLE[key + 3];
  return undefined;
}
...
export function getTotalCodewordsCount(
  ...
  const key = (version - 1) * 4;
  if (errorCorrectionLevel === L) return EC_CODEWORDS_TABLE[key + 0];
  if (errorCorrectionLevel === M) return EC_CODEWORDS_TABLE[key + 1];
  if (errorCorrectionLevel === Q) return EC_CODEWORDS_TABLE[key + 2];
  if (errorCorrectionLevel === H) return EC_CODEWORDS_TABLE[key + 3];
  return undefined;
}
